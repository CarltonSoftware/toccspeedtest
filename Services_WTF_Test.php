...
class Services_WTF_Test {
    ...
    public function resources( $item = 'all' ) {
        ...
        if (isset($this->result['resources'][$item])) {
            return $this->result['resources'][$item];
        }
     ...
     * Downloads test resources to a specified location
     *
     * $item string item(s) to download (empty or null will result in all resources downloading)
     * 
     * returns true if successful, the error message if an error occured
     */
    public function download_resource($item)
    {
        if ( !$this->completed() ) {
            return false;
        }
